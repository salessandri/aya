...
/// Ring buffer map.
///
/// Implements the abstraction layer for [ringbuf](https://github.com/torvalds/linux/blob/master/kernel/bpf/ringbuf.c).
/// It provides two mechanisms to write an entry in the ring buffer: copying an structure into the
/// buffer and reserve-then-commit. If using the latter, no memory copying is needed.
///
/// More information in the [kernel's BPF ring buffer documentation](https://www.kernel.org/doc/html/latest/bpf/ringbuf.html).
///
...
///
/// #[map]
/// static mut EVENTS_RINGBUF: RingBuf = RingBuf::with_max_entries(1 << 16, 0);
///
/// fn try_example_one(ctx: TracePointContext) -> Result<u32, u32> {
///
///     let event = SomeEvent{
///         a: 1,
///         b: 2,
...
///     };
///
///     unsafe { EVENTS_RINGBUF.output(&event, 0).map_err(|_| 1u32)?; }
///
///     Ok(0)
/// }
///
/// fn try_example_two(ctx: TracePointContext) -> Result<u32, u32> {
///
///     let mut event_entry = unsafe {
///         PROGRAM_EVENTS.reserve::<SomeEvent>(0).ok_or(1u32)?
///     };
///     unsafe {
///         *event_entry.as_mut_ptr() = SomeEvent {
///             a: 1,
///             b: 2,
///             c: -10
///         };
...
///     if pid % 2 == 0 {
///         event_entry.submit(0);
///     }
///     else {
///         event_entry.discard(0);
///     }
///
///     Ok(0)
/// }
...
impl<T> RingBufEntry<T> {
    /// Discard this ring buffer entry.
    ///
    /// The entry will be skipped by the userspace reader.
    pub fn discard(self, flags: u64) {
        unsafe { bpf_ringbuf_discard(self.0.as_mut_ptr() as *mut _, flags) };
    }

    /// Commit this ring buffer entry.
    ///
    /// The entry will be made visible to the userspace reader.
...
impl RingBuf {
    ...
    /// Until this happens, the consumer will not not be allowed to advance.
    ///
